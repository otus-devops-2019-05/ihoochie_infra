...
[ДЗ №9 Деплой и управление конфигурацией с Ansible](#дз-9-деплой-и-управление-конфигурацией-с-ansible)
* [Один плейбук, один сценарий](#один-плейбук-один-сценарий)
...
* Создан плейбук reddit_app_one_play.yml, описывающий конфигурацию mongodb и сервера приложения
* Отдельные таски запускаются с указанием в команде запуска аргументов --limit (на каких хостах запусать) и --tags(какие таски запускать)
* Такой подход является неудобным, потому что нужно помнить на каких хостах и какие таски нужно запускать. Читать такой конфиг трудно.

##### Один плейбук, несколько сценариев

* Создан плейбук reddit_app_multiple_plays.yml, в котором задания разбиты на несколько таксов
* Теги и к выполнение под root перенесены на уровень тасков
* Тако конфиг удобнее читать и в команде нужно передавать только название тега
  ```
  $ ansible-playbook reddit_app_multiple_plays.yml --tags db-tag
  ```
* Такой конфигурацие легче управлять, но при увеличении количества управляемых ресурсов плейбук становится объемным и трудночитаемым

##### Несколько плейбуков

* Задания разделены на несколько плейбуков: app.yml, db.yml и deploy.yml
* Созданные плейбуки вынесены в основной - site.yml
* Такой конфигурацией стало легче управлять

##### Провижининг в Packer

* Изменен провижининг в конфигурации создания образов в Packer
* Использование скриптов заменено на плейбуки, в которых используютя модули Ansible: packer_db.yml и packer_app.yml
* packer/db.json
  ```
  "provisioners": [
      {
...
* Создаем инфраструктуру из terraform/stage
  ```
  $ terraform apply
  ```
* Запускаем основной плейбук для деплоя приложения
  ```
